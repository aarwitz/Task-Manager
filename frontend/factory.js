...
els.createIssueBtn.onclick = () => openModal(true); els.closeModalBtn.onclick = () => openModal(false); els.cancelModalBtn.onclick = () => openModal(false); els.closeDrawerBtn.onclick = closeDrawer; els.logoutBtn.onclick = () => { localStorage.removeItem('username'); window.location.href = '/'; }; els.toggleHudBtn.onclick = () => { els.hudPanels.classList.toggle('hidden'); els.toggleHudBtn.textContent = els.hudPanels.classList.contains('hidden') ? 'Show task panels' : 'Hide task panels'; }; window.addEventListener('click', e => { if (e.target === els.createIssueModal) openModal(false); });
...
function renderMobileWorld(running, human, queued, review, done){
  const live = [...running, ...human, ...review].slice(0, 4);
  els.factoryWorld.innerHTML = `
    <div class="mobile-world">
      <div class="mobile-scene-top">
        <div class="mobile-crew-row big-crew">
          <div class="mobile-operator hero-op agent" data-owner="Jerry">${sprite('Jerry','agent', 0, 0, groupByOwnerCounts('Jerry'))}</div>
          <div class="mobile-operator hero-op human" data-owner="Aaron">${sprite('Aaron','human', 0, 0, groupByOwnerCounts('Aaron'))}</div>
          <div class="mobile-operator hero-op human" data-owner="Taylor">${sprite('Taylor','human', 0, 0, groupByOwnerCounts('Taylor'))}</div>
        </div>
        <div class="mobile-action-strip">
          ${live.map((issue, idx) => `<article class="mobile-scene-task ${taskTheme(issue).cls}" data-id="${issue.id}"><span class="mobile-scene-icon">${taskGlyph(issue)}</span><div><strong>${escapeHtml(shortTitle(issue.title, 20))}</strong><span>${escapeHtml(canonOwner(issue.assigned_to || ''))}</span></div></article>`).join('') || '<div class="empty-block">No live builds.</div>'}
        </div>
      </div>
      <div class="mobile-zones game-zones">
