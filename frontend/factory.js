...
async function resolveSprint(){ if (state.sprintId) { try { return await api(`/api/sprints/${state.sprintId}`); } catch {} } try { return await api('/api/sprints/active'); } catch { const all = await api('/api/sprints'); return all[0] || null; } }
function needsHuman(issue){ const owner = (issue.assigned_to || '').toLowerCase(); return owner.includes('aaron') || owner.includes('human'); }
function actorClass(name){ const n = (name || '').toLowerCase(); if (!name) return 'unassigned'; if (['claw','agent','bot','subagent','codex'].some(x => n.includes(x))) return 'agent'; return 'human'; }
...
function card(issue, emphasize=false){ const owner = issue.assigned_to || 'Unassigned'; const cls = actorClass(owner); return `<article class="run-card ${emphasize ? 'needs-human' : ''}" data-id="${issue.id}"><div class="run-top"><div class="chip-row"><span class="chip ${cls}">${escapeHtml(owner)}</span>${emphasize ? '<span class="chip needs">Needs Aaron</span>' : ''}</div><span class="run-id">#${issue.id}</span></div><h4>${escapeHtml(issue.title)}</h4><div class="run-bottom"><span class="chip">${statusMeta[issue.status].label}</span><span class="run-time">${relTime(issue.created_at)}</span></div></article>`; }
function renderLane(el, issues, emphasize=false){ el.innerHTML = issues.length ? issues.map(i => card(i, emphasize)).join('') : '<div class="empty-block">No runs here.</div>'; el.querySelectorAll('.run-card').forEach(node => node.onclick = () => { const issue = state.issues.find(i => i.id === Number(node.dataset.id)); if (issue) openDrawer(issue); }); }
...
els.commentForm.onsubmit = async (e) => {
  ...
  const original = els.addCommentBtn.textContent;
  els.addCommentBtn.disabled = true;
  els.addCommentBtn.textContent = 'Adding…';
  try {
    await api(`/api/issues/${state.selectedIssue.id}/comments`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ content, username }) });
    await sync();
    const refreshed = state.issues.find(i => i.id === state.selectedIssue.id);
    ...
    els.addCommentBtn.textContent = original;
  }
};
